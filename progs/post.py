...
BASE_URL = 'http://192.168.2.87:8080'
'''
...
device['charging'] = 1
device['System uptime [s]'] = 12
device['App uptime [s]'] = 21
...
device['EDAG Busv. [V]'] = -92.5
device['charging'] = 1
device['System uptime [s]'] = 34
device['App uptime [s]'] = 43
devices.append(device)
...
device['System uptime [s]'] = 56
device['App uptime [s]'] = 65
devices.append(device)
...
device['Power usage [kWh]'] = 384.15
device['Power [kW]'] = 0.38
device['System uptime [s]'] = 78
device['App uptime [s]'] = 87
devices.append(device)
...
while True:
    for dev in devices:
      ...
      try:
        ...
        res = requests.get(f'{BASE_URL}/sendDevInfo')
        #print(f'### {res} -- {type(res.json())} - {res.json()}')
      except:
